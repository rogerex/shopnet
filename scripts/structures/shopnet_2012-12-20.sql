...
/* Created on:     12/20/2012 3:41:52 PM                        */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('DETAIL_PURCHASE') and o.name = 'FK_DETAIL_P_STOCKS_PR_PRODUCT')
alter table DETAIL_PURCHASE
   drop constraint FK_DETAIL_P_STOCKS_PR_PRODUCT
go
...
if exists (select 1
           ...
           where  id    = object_id('DETAIL_PURCHASE')
            ...
            and   indid > 0
            and   indid < 255)
   drop index DETAIL_PURCHASE.STOCKS_PRODUCT_FK
go
...
create table DETAIL_PURCHASE (
   ...
   COST_PURCHASE        decimal              not null,
   PRICE_PURCHASE       decimal              not null,
   TOTAL_AMOUNT_PURCHASE int                  not null,
   constraint PK_DETAIL_PURCHASE primary key nonclustered (ID_PRODUCT, ID_PURCHASE)
)
go
...
/*==============================================================*/
create index STOCKS_PRODUCT_FK on DETAIL_PURCHASE (
ID_PRODUCT ASC
)
go
...
alter table DETAIL_PURCHASE
   add constraint FK_DETAIL_P_STOCKS_PR_PRODUCT foreign key (ID_PRODUCT)
